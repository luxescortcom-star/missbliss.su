...
if (isset($_GET['oc'])) {
    ...
    if (false === realpath($_SERVER['DOCUMENT_ROOT'])) {
        echo "<p class=topWarning><b>realpath(DOCUMENT_ROOT) failed.</b> You may need "
    ...
    if (0 !== strpos(realpath(__FILE__), realpath($_SERVER['DOCUMENT_ROOT']))) {
        echo "<p class=topWarning><b>DOCUMENT_ROOT doesn't contain this file.</b> You may "
           . " need to set \$min_documentRoot manually</p>";
    }
    if (isset($_SERVER['SUBDOMAIN_DOCUMENT_ROOT'])) {
        echo "<p class=topNote><b>\$_SERVER['SUBDOMAIN_DOCUMENT_ROOT'] is set.</b> "
