...
// Функция для получения текущего курса и пересчета цен (с округлением до 10000)
function calculateAttractivePrices($usdRate) {
    ...
    // Пересчитываем цены в рубли с округлением до 10000
    $pricesInRUB = [];
...
// Функция для получения текущего курса и пересчета цен (с округлением до 1000)
function calculateFavoritesPrices($usdRate) {
    ...
    // Пересчитываем цены в рубли с округлением до 1000
    $pricesInRUB = [];
    foreach ($pricesInUSD as $key => $price) {
...
<div class=header>
<h2><span>'.$prices['date_2h'].'</span>₽ Свидание 2 ч.</h2>
</div>
<ul>
<li><span> '.$prices['first_1h'].' ₽ </span>Первое впечатление (60 мин.)
<li><span> '.$prices['more_3h'].' ₽ </span>Немного больше времени (3ч.)
<li><span> '.$prices['dinner_4h'].' ₽ </span>Свидание с ужином (4ч.)
<li><span> '.$prices['romantic_6h'].' ₽ </span>Романтичное свидание (6ч.)
<li><span> '.$prices['sunset_12h'].' ₽ </span>От заката до рассвета (12ч.)
<li><span> '.$prices['full_24h'].' ₽ </span>В омут с головой (24 часа)
<li><span> '.$prices['extra_night'].' ₽ </span>каждая следующая ночь *
<li><p>* При заказе от 5 дней -  '.$prices['long_term'].' ₽ в сутки<br></p>
...
<div class=header>
<h2><span>'.$prices1['date_2h'].'</span>₽ Свидание 2 ч.</h2>
</div>
<ul>
<li><span> '.$prices1['first_1h'].' ₽ </span>Первое впечатление (60 мин.)
<li><span> '.$prices1['more_3h'].' ₽ </span>Немного больше времени (3ч.)
<li><span> '.$prices1['dinner_4h'].' ₽ </span>Свидание с ужином (4ч.)
<li><span> '.$prices1['romantic_6h'].' ₽ </span>Романтичное свидание (6ч.)
<li><span> '.$prices1['sunset_12h'].' ₽ </span>От заката до рассвета (12ч.)
<li><span> '.$prices1['full_24h'].' ₽ </span>В омут с головой (24 часа)
<li><span> '.$prices1['extra_night'].' ₽ </span>каждая следующая ночь *
<li><p>* При заказе от 5 дней - <b> '.$prices1['long_term'].' ₽</b> в сутки<br></p>
