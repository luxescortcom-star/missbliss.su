 ...
 */
function generateSlider($flink, $name3, $folder, $name ) {
    // Путь к папке с изображениями
   $baseDir = $_SERVER['DOCUMENT_ROOT'] . "/fans-only"; // Пример: /var/www/site/images
    $imageDir = "$baseDir/$name3";
    ...
    foreach ($images as $image) {
        ...
        $html .= '
        <li>
            <a href="https://' . $flink . '/' . $name3 . '/' . $imageName . '.' . $ext . '" class="pP[l]" title="' . $name . '"><img class=get  data-src="https://' . $flink . '/' . $name3 . '/' . $folder . '/' . $imageName . '.' . $ext . '" alt="' . $name . '">
            </a>
        </li>';
    }
    $html .= '</ul>';
...
echo generateSlider($flink, $name3, $folder,  $name);
?>