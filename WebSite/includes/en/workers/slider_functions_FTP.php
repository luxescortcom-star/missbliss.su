<?php
function generateSlider($flink, $blink, $picsFolio, $folder, $folder_full, $name, $baseDir = null) 
{
     // Настройки FTP
    ...
    // Локальная папка (если FTP недоступен)
    if ($baseDir === null) {
        $baseDir = $_SERVER['DOCUMENT_ROOT'] . "/$blink";
    }
    $localDir = "$baseDir/$picsFolio/$folder";
    ...
    foreach ($uniqueImages as $imageName => $image) {
        if ($isFirst) {
            ...
            $html .= <<<HTML
<li>
    <a href="https://{$flink}/{$picsFolio}/{$folder_full}/{$imageName}.{$selectedExtension}" class="pP[l]" title="{$name}">
        <img src="https://{$flink}/{$picsFolio}/{$folder}/{$imageName}.{$selectedExtension}" alt="{$name}">
    </a>
</li>
HTML;
...
<li>
    <a href="https://{$flink}/{$picsFolio}/{$folder_full}/{$imageName}.{$selectedExtension}" class="pP[l]" title="{$name}">
        <img class="get" data-src="https://{$flink}/{$picsFolio}/{$folder}/{$imageName}.{$selectedExtension}" alt="{$name}">
    </a>
</li>
HTML;
        }
...
echo generateSlider($flink, $blink, $picsFolio, $folder, $folder_full, $name);
?>