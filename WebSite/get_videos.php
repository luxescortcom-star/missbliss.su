...
$city = $_GET['city'] ?? '';
$name = $_GET['name'] ?? '';
$videoDir = __DIR__ . "/images/$city/$name/video/";
$cacheFile = __DIR__ . "/images/$city/$name/video_cache.json";
...
if (file_exists($cacheFile) && (time() - filemtime($cacheFile) < 3600)) {
    ...
    foreach (scandir($videoDir) as $file) {
        if (preg_match('/\.(mp4|mov|avi|webm)$/i', $file)) {
            $videos[] = [
                'url' => "/images/$city/$name/video/$file",  // Относительный путь от корня сайта
                'name' => $file,
            ];
        }
    }
