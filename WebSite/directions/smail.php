...
if(trim($name) == 'Ваше Имя') {
   ...
   echo '<div class="error_message">Пожалуйста, в поле Имя - пишите только имя, ваше сообшение пишите в поле ниже</div>';
	...
	exit();
} else if((trim($comments) == 'Сообщение')OR(trim($comments) == '')OR(mb_strlen($comments) < 7)) {
	echo '<div class="error_message"><a href=#mail title="Как быстро получить ответ?">Пожалуйста, напишите подробно ваше сообщение </a></div>';
	exit();
