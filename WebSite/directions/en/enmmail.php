...
if(trim($name) == 'Name') {
	...
	echo '<div class="error_message">You are not allowed to write.</div>';
	exit();
}
else if(mb_strlen($name) > 15)  {
   echo '<div class="error_message">Please, in the Name field - write only your name, write your message in the field below</div>';
	exit();
}else if(trim($email) == 'E-mail') {
	echo '<div class="error_message">Please, enter your e-mail address.</div>';
	exit();
} else if(trim($phone) == '') {
	...
	echo '<div class="error_message">You entered an incorrect email. Please, try again.</div>';
	exit();
	...
	echo '<div class="error_message">Your address is blacklisted</div>';
	exit();
} else if((trim($comments) == 'Message')OR(trim($comments) == '')) {
	echo '<div class="error_message">Please write in detail your wishes.<a href=#rules title="How to make an order quickly?">How to make an order quickly?</a></div>';
	exit();
} else if(mb_strlen($comments) > 300) {
   echo '<div class="error_message">Please write briefly your wishes</div>';
	exit();

}else  if (isset($recaptcha_data->success) && $recaptcha_data->success === FALSE) {
echo '<div class="error_message">You dont look human</div>';
	exit();
...
die('<div class="error_message">Spam and links are prohibited. <a href=#spam title="Letter rules">Detailed</a></div>'); //убиваем скрипт и выводим сообщение
}
...
$e_subject = 'from missbliss.ru/en/contact Moscow ' . $name . '.';
...
$e_body = "Moscow Guest: $name" . PHP_EOL . PHP_EOL;
