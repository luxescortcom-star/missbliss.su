...
// Проверка имени
if(!isValidName($name)) {
    echo '<div class="error_message">Please enter a valid name (letters only, no numbers).</div>';
...
if(!filter_var($email, FILTER_VALIDATE_EMAIL)) {
    echo '<div class="error_message">You entered an incorrect email address. Please try again.</div>';
    logFailed($logData, 'Неверный формат e-mail');
    exit();
}

if(!verifyEmail($email)) {
    echo '<div class="error_message">You entered an invalid email address. Please try again.</div>';
    logFailed($logData, 'Несуществующий e-mail');
    exit();
}

if($name == '' || $name == 'Youre Name') {
...
if(isBlacklisted($email, $comments)) {
    echo '<div class="error_message">Your address or message is blacklisted.</div>';
    logFailed($logData, 'Email или сообщения в черном списке');
    exit();
}

if((trim($comments) == 'Your brief CV') || (trim($comments) == '')) {
    echo '<div class="error_message">Please, send Youre a brief CV. <a href=#mail title="What to write about yourself?">What to write about yourself?</a></div>';
    exit();
}

if(mb_strlen($comments) > 300) {
   echo '<div class="error_message">Please write a little shorter. - <a href=#mail title="Required resume">Required resume?</a></div>';
    exit();
}
...
try {
    ...
    if (sendToTelegram(['name' => $name, 'email' => $email, 'phone' => $phone, 'comments' => $comments])) {
        ...
        echo "<p>Your request has been received! We will contact you within 2 hours.</p>";
        echo "<p>If you need an urgent response, please contact us on Telegram:</p>";
        echo "<p><a href='https://t.me/TheMissBlissAgency' style='color: #0088cc; text-decoration: none;' target='_blank'>📱 Write to Telegram</a></p>";
        echo "</div>";
        echo "</fieldset>";
    } else {
        logFailed($logData, 'Error sending to Telegram');
        echo '<div class="error_message">Failed to submit your application. Please contact us on Telegram: @TheMissBlissAgency</div>';
    }
} catch (Exception $e) {
    logFailed($logData, 'Исключение: ' . $e->getMessage());
    echo '<div class="error_message">There was an error processing your request. Please try again later or contact us on Telegram.div>';
