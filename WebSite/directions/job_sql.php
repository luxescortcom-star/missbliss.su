...
define('DB_USER', 'if0_40406087'); // ваш username
define('DB_PASS', 'Graniate795'); // замените на ваш пароль
...
// Функция для сохранения заявки модели
function saveModelToDatabase($data) {
    ...
    try {
        ...
        if ($result) {
            error_log("Successfully saved model application to database");
            return true;
        } else {
            error_log("Model database insert failed");
            return false;
        }
...
if((trim($comments) == 'Ваше резюме') || (trim($comments) == '')) {
    echo '<div class="error_message">Напишите коротко о себе <a href=#mail title="Что написать о себе?">Что написать о себе?</a></div>';
    exit();
}
...
try {
    ...
    if (sendToTelegram(['name' => $name, 'email' => $email, 'phone' => $phone, 'comments' => $comments])) {
       ...
       $dbSuccess = saveModelToDatabase($logData);
        if (!$dbSuccess) {
