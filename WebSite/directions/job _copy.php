...
function isPhone($phone)
{
	...
	$phone = preg_replace('/\s|\+{2}|-|\(|\)/','', $phone); // удалим пробелы, и прочие не нужные знаки
...
if(trim($name) == 'Ваше Имя') {
   ...
   echo '<div class="error_message">Пожалуйста, в поле Имя - пишите только имя, ваше резюме пишите в поле ниже</div>';
	...
	exit();
} else if((trim($email) == 'clochykidke2024@mail.ru')OR(trim($email) == 'buspwhatsili2021@mail.ru')OR(trim($email) == 'myssoliniya@mail.ru')OR(trim($email) == 'regiyu@inbox.ru')OR(trim($email) == 'ella.pavlikova@yandex.ru')OR(trim($email) == 'sunnaq@mail.ru')OR(trim($email) == 'mariana.skvortsovskaya@yandex.ru')OR(trim($email) == 'jaclkmy@mail.ru')OR(trim($email) == 'svinya.svintus@yandex.ru')OR(trim($email) == 'darinaseotext@i.ua')OR(trim($email) == 'micky_catignani82@rambler.ru')OR(trim($email) == 'fsb-sasai@secmail.pro')) {
	echo '<div class="error_message">Ваш адрес в черном списке.</div>';
	exit();
}else if((trim($comments) == 'Ваше резюме')OR(trim($comments) == '')) {
	echo '<div class="error_message">Напишите коротко о себе <a href=#mail title="Что написать о себе?">Что написать о себе?</a></div>';
	exit();
} else if(mb_strlen($comments) < 15) {
   echo '<div class="error_message">Пожалуйста, напишите подробнее о себе - <a href=#mail title="Необходимое резюме">Что лучше написать?</a></div>';
	exit();
...
$e_subject = "По работе: $name $city_code";
...
$e_body = "$name, $city_code,  $countryName" . PHP_EOL . PHP_EOL;

$e_reply = "E-mail: $email\r\n Телефон: $phone\r\n $city_code,  $countryName\r\n $ip";
