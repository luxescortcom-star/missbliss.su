...
// Функция для подключения к базе данных через MySQLi
...
// Функция для сохранения в базу данных КЛИЕНТОВ
function saveClientToDatabase($data) {
    ...
    try {
        $sql = "INSERT INTO feedback (name, email, phone, message, ip_address, user_agent, city, region, country, request_type) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'client')";
        ...
        if ($result) {
            error_log("Successfully saved CLIENT to database");
            return true;
        } else {
            error_log("CLIENT database insert failed");
            return false;
        }
...
// Проверка имени
if(!isValidName($name)) {
    echo '<div class="error_message">Введите допустимое имя (только буквы, без цифр).</div>';
...
if(!filter_var($email, FILTER_VALIDATE_EMAIL)) {
    echo '<div class="error_message">Вы ввели неправильный адрес электронной почты. Попробуйте еще раз.</div>';
    logFailed($logData, 'Неверный формат e-mail');
    exit();
}

if(!verifyEmail($email)) {
    echo '<div class="error_message">Вы ввели несуществующий адрес электронной почты. Попробуйте еще раз.</div>';
...
if(isBlacklisted($email, $comments)) {
    echo '<div class="error_message">Ваш адрес или сообщение в черном списке.</div>';
    logFailed($logData, 'Email или сообщения в черном списке');
    exit();
}

if((trim($comments) == 'Сообщение') || (trim($comments) == '')) {
    echo '<div class="error_message">Напишите ваше сообщение: <a href=#mail title="Что написать?">Что написать?</a></div>';
    exit();
}

if(mb_strlen($comments) > 300) {
    echo '<div class="error_message">Пожалуйста, пишите короче: <a href=#mail title="Что написать?">Что написать?</a></div>';
    exit();
}
...
function sendToTelegram($data) {
    ...
    $chat_id = "-1003250816386"; // ID вашей группы Andey'sMembers
...
try {
    ...
    if (sendToTelegram(['name' => $name, 'email' => $email, 'phone' => $phone, 'comments' => $comments])) {
        // Логируем успех в файл
        logSuccess($logData);
        
        // Сохраняем в базу данных
        $dbSuccess = saveClientToDatabase($logData);
        if (!$dbSuccess) {
            error_log("Failed to save to database, but Telegram sent successfully");
            // Не прерываем выполнение, так как Telegram отправлен успешно
        }
        
        echo "<fieldset>";
        echo "<div id='success_page'>";
        echo "<h1>Спасибо, <strong>$name</strong>!</h1>";
        echo "<p>Ваша сообщение получено! Мы свяжемся с вами как можно быстрее.</p>";
        echo "<p>Если нужен срочный ответ, напишите нам в Telegram:</p>";
        echo "<p><a href='https://t.me/TheMissBlissAgency' style='color: #0088cc; text-decoration: none;' target='_blank'>📱 Написать в Telegram</a></p>";
        echo "</div>";
        echo "</fieldset>";
    } else {
        logFailed($logData, 'Ошибка отправки в Telegram');
        echo '<div class="error_message">Не удалось отправить заявку. Пожалуйста, напишите в Telegram: @TheMissBlissAgency</div>';
    }
} catch (Exception $e) {
    logFailed($logData, 'Исключение: ' . $e->getMessage());
    echo '<div class="error_message">Произошла ошибка при обработке заявки. Пожалуйста, попробуйте позже или напишите нам в Telegram.</div>';
