...
if(trim($name) == 'Имя') {
	echo '<div class="error_message">Пожалуйста, впишите Ваше имя.</div>';
	exit();
} else if((trim($name) == 'XRumerTest') || (trim($name) == 'NeooGaink') || (trim($name) == 'GraffRef') || (trim($name) == 'MichaelSmups') || (trim($name) == 'MichaelBlued') || (trim($name) == 'MichaelSmups') || (trim($name) == 'крео') || (trim($name) == 'www')) {
	echo '<div class="error_message">Вам запрещено писать.</div>';
	exit();
}
else if(mb_strlen($name) > 15)  {
   echo '<div class="error_message">Пожалуйста, в поле Имя - пишите только имя, ваше резюме пишите в поле ниже</div>';
	exit();
}else if(trim($email) == 'E-mail') {
	echo '<div class="error_message">Пожалуйста, впишите e-mail адрес.</div>';
	exit();
} else if(trim($phone) == '') {
	...
	exit();
 } else if((trim($phone) == '+7 916 680-64-10')OR(trim($phone) == '+7+900+900-90-90')OR(trim($phone) == '+7 900 900-90-90')) {
	die('<div class="error_message">Упс -  <a href=#spam title="Обработка писем">Похоже вы спаммер.</a></div>');
	exit();	
} else if(!isEmail($email)) {
	echo '<div class="error_message">Вы ввели неверный e-mail адрес. Попробуйте ещё раз.</div>';
	exit();
} else if((trim($email) == 'clochykidke2024@mail.ru') || (trim($email) == 'buspwhatsili2021@mail.ru') || (trim($email) == 'myssoliniya@mail.ru') || (trim($email) == 'regiyu@inbox.ru') || (trim($email) == 'ella.pavlikova@yandex.ru') || (trim($email) == 'sunnaq@mail.ru') || (trim($email) == 'mariana.skvortsovskaya@yandex.ru') || (trim($email) == 'jaclkmy@mail.ru') || (trim($email) == 'svinya.svintus@yandex.ru') || (trim($email) == 'darinaseotext@i.ua') || (trim($email) == 'micky_catignani82@rambler.ru') || (trim($email) == 'fsb-sasai@secmail.pro')) {
	echo '<div class="error_message">Ваш e-mail в черном списке.</div>';
	exit();
} else if((trim($comments) == 'Ваше резюме') || (trim($comments) == '')) {
	echo '<div class="error_message">Напишите о себе <a href=#mail title="Что написать о себе?">Что писать?</a></div>';
	exit();
} else if(mb_strlen($comments) > 300) {
   echo '<div class="error_message">Пожалуйста, чуть короче напишите - <a href=#mail title="Необходимое резюме">Что лучше написать?</a></div>';
	exit();
...
else if (!preg_match ("/href|url|базы|Jetmailone|ба3ы|данных|Рассылки|МИИТа|липовый|Регистрация|Свитмода|трикотаж|ключ|недорого|Bitrix|недвижимость|Предлагаем|домофон|Прокип|продукт|продвижение|контактным-формам|kontakt-forma|питание|СПОРТПИТ|поставщик|DedicateT|ВЫИГРАЛИ|qанных|bazy|dannyh|http|https|www|.ru|.com|.cn|.net|bit.ly|.info|.ly|.org/i", $comments))  {
//ссылок не найдено,
}
else {
die('<div class="error_message">Спам и ссылки -  <a href=#spam title="Обработка писем">Запрещены.</a></div>'); //убиваем скрипт и выводим сообщение
}


if (!preg_match ("/Перезвоните|перезвоните|все([[:space:]]|[[:punct:]])по([[:space:]]|[[:punct:]])телефону|позвоните|Позвоните/i", $comments) ) {
//ссылок не найдено,
}
else {
...
$e_subject = 'missbliss.ru/casting/ Санкт-Петербург' . $name . '.';
...
$e_body = "Новенькая модель в Санкт-Петербурге: $name" . PHP_EOL . PHP_EOL;
