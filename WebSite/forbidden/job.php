...
if(trim($name) == 'Имя') {
   ...
   echo '<div class="error_message">Ошибка. В поле Имя - пишите только имя, ваше сообшение пишите в поле ниже</div>';
	exit();
	...
	echo '<div class="error_message">Напишите кратко о себе<a href=#mail title="Как быстро получить ответ?">Что написать?</a></div>';
	exit();
} else if(mb_strlen($comments) > 300) {
   echo '<div class="error_message">Слишком много текста. Пишите кратко ваше резюме.</div>';
	exit();
...
die('<div class="error_message">Спам и ссылки запрещены. <a href=#fee title="Обработка писем">Подробнее</a></div>'); //убиваем скрипт и выводим сообщение
}
...
die('<div class="error_message">Мы не перезваниваем. <a href=#call title="Правила"> Подробнее</a></div>'); //убиваем скрипт и выводим сообщение
}
...
$e_subject = 'missbliss.ru/casting ' . $name . '.';
...
$e_body = "Новенькая модель: $name" . PHP_EOL . PHP_EOL;
