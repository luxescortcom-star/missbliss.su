...
if(trim($name) == 'Имя') {
	...
	exit();
} else if(trim($phone) == 'Номер телефона') {
	echo '<div class="error_message">напишите ваш номер для связи</div>';
	exit();
} else if(!isPhone($phone) =='false') {
	echo '<div class="error_message">Напишите ваш номер для связи</div>';
	exit();
} else if(strlen($phone) == 'false') // если длина номера слишком короткая, вернем false
		{
	echo '<div class="error_message">Вы ошиблись при вводе номера телефона, проверьте</div>';
	exit();
	...
	exit();
} else if(trim($comments) == 'Сообщение') {
	echo '<div class="error_message">Напишите свое короткое резюме  <a href=#mail title="Что написать о себе?">Что написать?</a></div>';
	exit();
}
