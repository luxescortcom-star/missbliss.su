...
if(trim($name) == 'Имя') {
	...
	exit();
} else if((trim($email) == 'clochykidke2024@mail.ru')OR(trim($email) == 'buspwhatsili2021@mail.ru')OR(trim($email) == 'darinaseotext@i.ua')OR(trim($email) == 'micky_catignani82@rambler.ru')) {
	...
	exit();
} else if (!preg_match ("/href|url|базы|ба3ы|данных|Рассылки|Предлагаем|контактным-формам|kontakt-forma|qанных|bazy|dannyh|http|https|www|.ru|.com|.cn|.net|.info|.org/i", $comments)) {
//ссылок не найдено,
} else {
