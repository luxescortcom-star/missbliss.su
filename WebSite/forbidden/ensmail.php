    ...
    if (!empty($token)) {
        if (hash_equals($_SESSION['token'], $token)) {
            ...
            if (isset($recaptcha_data->success) && $recaptcha_data->success === TRUE) {
                ...
                $success = "You're a human!"; // Not of a production server:
            }
        } else {
...
if(trim($name) == 'Name') {
	...
	echo '<div class="error_message">Please, enter youre correct phone number</div>';
	exit();
} else if(!isEmail($email)) {
	echo '<div class="error_message">You have enter an invalid e-mail address, try again</div>';
	exit();
} else if((trim($email) == 'clochykidke2024@mail.ru')OR(trim($email) == 'buspwhatsili2021@mail.ru')OR(trim($email) == 'myssoliniya@mail.ru')OR(trim($email) == 'regiyu@inbox.ru')OR(trim($email) == 'ella.pavlikova@yandex.ru')OR(trim($email) == 'sunnaq@mail.ru')OR(trim($email) == 'mariana.skvortsovskaya@yandex.ru')OR(trim($email) == 'jaclkmy@mail.ru')OR(trim($email) == 'svinya.svintus@yandex.ru')OR(trim($email) == 'darinaseotext@i.ua')OR(trim($email) == 'micky_catignani82@rambler.ru')) {
	echo '<div class="error_message">Your e-mail address is blacklisted</div>';
	exit();
}
else if(trim($comments) == 'Message') {
	echo '<div class="error_message">Please write about yourself.<a href=#rules title="What to write about yourself?"> What to write about?</a></div>';
	exit();
	} else  if (isset($recaptcha_data->success) && $recaptcha_data->success === FALSE) {
echo '<div class="error_message">You not a Human</div>';
	exit();
}
else if (!preg_match ("/href|url|базы|ба3ы|данных|Рассылки|Перезвоните|перезвоните|позвоните|Предлагаем|контактным-формам|kontakt-forma|DedicateT|ВЫИГРАЛИ|qанных|bazy|dannyh|http|https|www|.ru|.com|.cn|.net|bit.ly|.info|.ly|.org/i", $comments)) {
//ссылок не найдено,
} else {
die('<div class="error_message">SPAM and Links are not allowed  <a href=#spam title="Email Processing Policy">Detailed</a></div>'); //убиваем скрипт и выводим сообщение
}
...
$e_subject = 'Newbies from St Petersburg missbliss.ru/en/casting , ' . $name . '.';
