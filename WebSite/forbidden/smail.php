...
if(trim($name) == 'Имя') {
   ...
   echo '<div class="error_message">Пожалуйста, в поле Имя - пишите только имя, ваше сообшение пишите в поле ниже</div>';
	...
	exit();
} else if((trim($email) == 'clochykidke2024@mail.ru')OR(trim($email) == 'Ronan330981@mail.ru')OR(trim($email) == 'maksim224@mail.ru')OR(trim($email) == 'buspwhatsili2021@mail.ru')OR(trim($email) == 'myssoliniya@mail.ru')OR(trim($email) == 'regiyu@inbox.ru')OR(trim($email) == 'ella.pavlikova@yandex.ru')OR(trim($email) == 'sunnaq@mail.ru')OR(trim($email) == 'mariana.skvortsovskaya@yandex.ru')OR(trim($email) == 'jaclkmy@mail.ru')OR(trim($email) == 'svinya.svintus@yandex.ru')OR(trim($email) == 'darinaseotext@i.ua')OR(trim($email) == 'micky_catignani82@rambler.ru')OR(trim($email) == 'fsb-sasai@secmail.pro')) {
...
$e_subject = 'missbliss.ru/contact Санкт-Петербург ' . $name . '.';
...
$e_body = "Из Санкт-Петербурга: $name" . PHP_EOL . PHP_EOL;
